...
export default function Restaurant({promoQrCode}: Props) {
  ...
  console.log('promoQrCode', promoQrCode)
  const handleSubmit = async (data: IFormData) => {
    try {
      setSending(true)
      await RestaurantOrderRepository.create({...omit(data, ['total']), total: parseInt(`${data.total}`, 10), code: promoQrCode.code})
      setSuccess(true)
    } catch (err: any) {
        ...
        <div className={styles.center}>
            ...
            {success && <div className={styles.success}><QrSuccess/></div>}
              {!success && <>
                ...
                <QrFormCard isSending={sending}/>
                <Button className={styles.btn} spinner={sending} styleType='filledGreen'
