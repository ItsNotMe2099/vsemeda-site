...
export default function PreOrderModal(props: Props) {
  ...
  const header = (
    <div className={styles.header}>
      {!isMobile&& 
      <><div className={styles.title}>Время доставки</div><Spacer basis={15}/></>
      }
      <TabsField name={'day'} wrapperClassName={styles.daysWrapper} rootClassName={styles.daysRoot} optionClassName={styles.daysOption} activeOptionClassName={styles.daysOptonActive}  options={optionsDays}/>
    </div>
  )
  ...
  const body = (<>
      <RadioListField
        ...
        name='time'
        label={!isMobile&&'Время'}
        labelClassName={styles.radioItem}
        ...
        {!isMobile && <Spacer basis={20}/>}
    </>
  )

  const footer = <Button onClick={()=>{formik.submitForm()}} spinner={loading} font='semibold16' className={styles.submitButton} styleType='filledGreen'>Выбрать</Button>

  if (props.isBottomSheet) {
    return (
      <FormikProvider value={formik}>
        <Form className={styles.root}>
          <BottomSheetLayout backgroundColor={isMobile&&colors.purple} closeIconColor={colors.grey2}>
            <BottomSheetBody>
